...
if (count($parts) === 3 && $parts[0] === 'ready-made') {
    $category = $parts[1];
    $product = $parts[2];

    // ✅ Load product page
    include('product-template.php');
    exit; // 🔴 Stop executing rest of this file
} elseif (count($parts) === 2 && $parts[0] === 'ready-made') {
    $category = $parts[1];
    ...
    exit;
        ...
        <div class="jumbotron--list-detail__col-half jumbotron--list-detail__col-right">
            ...
            <div class="gap-bottom-small">
                <h1 class="jumbotron__title"><?php echo $_SESSION['category']; ?> EMAIL LIST</h1>
                <strong class="jumbotron--list-detail__subtitle"></strong>
            </div>
            <div class="gap-bottom">
                <span class="jumbotron--list-detail__price"  style="color: orange">
                 <?php echo '$ '.number_format($_SESSION['price']); ?>
                </span>
                <h4 class="jumbotron__title">Custom Order E-mail: <span id="demo" class="jumbotron--list-detail__price" style="color: orange"><?php  echo number_format($_SESSION['total_email']); ?></span></h4>
                <h4 class="jumbotron__title">Total Price: <span id="demo1" class="jumbotron--list-detail__price" style="color: #FA121A"><?php echo '$ '.number_format($_SESSION['price']); ?></span></h4>
                <div class="slidecontainer">
                    <input type="range" min="1000" max="<?php
                       echo $_SESSION['total_email'];
                        ?>" value="<?php echo $_SESSION['total_email'];
                        ?>" class="slider" id="myRange" step="100">
                    
                </div>
            </div>
            <p class="text-loblolly">If you want to make stronger connections with other businesses, your goal should be to get in touch with the real decision-makers at companies. <?php echo $_SESSION['short_description'] ;
?>!</p>
            <div class="gap-bottom-medium hidden-tlnd">
                <form action="<?= $siteUrl; ?>checkout/step1" method="POST">
                    <input type="hidden" name="ordercode" value="<?php echo ('MI' . rand(10, 99) . 'C' . rand(10, 99) . 'K' . rand(0, 9) . 'Y'); ?>" >
                    <input type="hidden" name="emailType" value="Ready Made" >
                    <input type="hidden" name="emailCategory" value="<?php
                    echo $_SESSION['title'];
                    ?>" >
                    <input type="hidden" name="selectItem" value="<?php
                    echo $_SESSION['category'];
                    ?>">
                    <input type="hidden" name="totalemail" value="<?php
                    echo $_SESSION['total_email'];
                    ?>" >
                    <input type="hidden" name="price" value="<?php
                    echo $_SESSION['price'];
                    ?>" >
                    <input type="hidden" class="form__control" name="deliveryDays" value="Today">
                    
                    <input type="hidden" name="dataType" id="dataType" >
                    
                    <span id="customPrice" style="color: black;"></span>
                    <input type="submit" class="button button--primary gap-right-plnu full-width-pld gap-bottom-small-pld" value="Buy Now" name="buyNow" >
                </form>
            </div>
            <ul class="list row">
    ...
    <div class="container">
        <div class="row gap-bottom-large">
            <div class="col-md-10 col-md-offset-1">
                <h2 class="primary-title h3"><?php
                    echo $_SESSION['seo_title'];
                    ?> </h2>
